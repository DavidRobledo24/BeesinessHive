...
const Actividad = () => {
  ...
  const {  actividad,  fecha, descripcion,idActividadApiario } = location.state || {}; 
  ...
  useEffect(() => {
    ...
    headers: {
        ...
        if(response.status == 403){
          ...
          else if(decodedToken.role === "productor" && !idActividadApiario) navigate("/gestionApiarios");
        }
    })
  }, []);
  ...
  const handleColmenaClick =async () => {
    ...
    await fetch(`http://54.87.25.132/actividadApiario/${idActividadApiario}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
      ...
      .then((response) => {
        ...
        Swal.fire({
          title: '¡Éxito!',
          text: 'Actividad del apiario eliminada correctamente.',
      ...
      .catch((error) => {
        Swal.fire({
          title: "No fue posible eliminar la actividad apiario por un error interno del servidor ",
          icon: "error"
      })
      });
